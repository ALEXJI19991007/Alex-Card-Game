...
const setupServer = async () => {
  ...
  if (env === "production") {
    ...
    server = https.createServer(options, app).listen(port, () => {
      console.log(`AlexSolitare listening on: ${server.address().port}`);
    });
    ...
    http
      ...
      .listen(80, () => {
        console.log(`AlexSolitare listening on 80 for HTTPS redirect`);
      });
  } else {
    server = app.listen(port, () => {
      console.log(`AlexSolitare ${env} listening on: ${server.address().port}`);
    });
  }
};
